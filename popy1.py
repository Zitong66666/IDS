...
class Level3_Data(dict):
    ...
    def fit_topography(self,mask=None,min_windtopo=None,max_windtopo=None,
        ...
        if self.instrum == 'TROPOMI' and self.product == 'NO2':
            ...
            outlier_std = outlier_std or 2
            fit_chem = fit_chem or False
        elif self.instrum == 'IASI' and self.product == 'NH3': # zitong add
            min_windtopo = min_windtopo or 0
            max_windtopo = max_windtopo or 1
            max_iter = max_iter or 2
            outlier_std = outlier_std or 2
            fit_chem = fit_chem or False
        else:
    ...
    def fit_chemistry(self,mask=None,min_windtopo=None,max_windtopo=None,
                      max_wind_column=None,max_iter=None,outlier_std=None, min_column_amount=None):
        '''infer lifetime for the chemical loss term
        '''
        import statsmodels.formula.api as smf
        import pandas as pd
        if self.instrum == 'IASI' and self.product == 'NH3': # zitong edit
            min_windtopo = min_windtopo or 0
            max_windtopo = max_windtopo or 0.001
            max_wind_column = max_wind_column or 0.03
            max_iter = max_iter or 2 # zitong edit
            outlier_std = outlier_std or 2
            min_column_amount = min_column_amount or 2.5e-9 # zitong add
        else:
            self.logger.debug('not supported for this product')
        ...
        ww = self['column_amount']
        chem_mask = (wt >= min_windtopo) & (wt <= max_windtopo) & (wc <= max_wind_column) & (ww >= min_column_amount) # zitong edit
        if mask is not None:
    ...
    def plot(self,plot_field=None,scale=None,
             ...
             layer_threshold=0.5,draw_colorbar=True,saving_path=None, #zitong add saving_path
             func=None,**kwargs):
        import matplotlib.pyplot as plt
        ...
        if func is not None:
            ...
            plotdata = self[plot_field]
        if 'cmap' not in kwargs.keys():
            kwargs['cmap'] = 'jet' #'rainbow'RdYlBu zitong edit
        if 'alpha' not in kwargs.keys():
            kwargs['alpha'] = 1.
        if 'shrink' not in kwargs.keys():
            kwargs['shrink'] = 0.75
        ...
        if draw_colorbar:
            ...
            cb = None
        plt.savefig(saving_path) #zitong add
        fig_output = {}
        fig_output['fig'] = fig
        fig_output['ax'] = ax
        fig_output['cb'] = cb
...
class Level3_List(list):
    ...
    def fit_topography(self,resample_rule=None,half_running_window=0,return_resampled=False,
        ...
        if resample_rule is None:
            ...
            for l3,(ind,sub_df) in zip(l3s_resampled,resampler.__iter__()): # l3 is monthly                
                try:
                    if nbootstrap is not None:
                        bootstrap_params = []
                        for i in range(nbootstrap):
                            ...
                            bootstrap_params.append(l3.topo_fit.params)
                    l3.fit_topography(**kwargs)
                except Exception as e:  # zitong add try and except here
                    continue
                ...
                for irow,row in sub_df.iterrows():
                    if l3.topo_fit is not None: # zitong add if~else
                        ...
                        l3.topo_fit.params['wt'] = np.nan
                        self[int(row['count'])].topo_fit.params['wt'] = np.nan
                        self[int(row['count'])]['wind_column_topo'] = np.nan
                        l3.topo_fit.mse_resid = np.nan
                        l3.topo_fit.rsquared = np.nan
        ...
        # zitong edit
        self.df['topo_scale_height'] = [-1/l3.topo_fit.params['wt'] if hasattr(l3, 'topo_fit') else np.nan for l3 in self] 
        self.df['topo_rmse'] = [np.sqrt(l3.topo_fit.mse_resid) if hasattr(l3, 'topo_fit') else np.nan for l3 in self]
        self.df['topo_r2'] = [l3.topo_fit.rsquared if hasattr(l3, 'topo_fit') else np.nan for l3 in self]
    ...
    def fit_chemistry(self,resample_rule=None,half_running_window=0,return_resampled=False,**kwargs):
        if resample_rule is None:
            ...
            for l3,(ind,sub_df) in zip(l3s_resampled,resampler.__iter__()):
                try:
                    ...
                    continue
                for irow,row in sub_df.iterrows():
                    if l3.chem_fit is not None: # zitong add if~else
                        ...
                        self[int(row['count'])].chem_fit.params['chem'] = np.nan
                        self[int(row['count'])]['wind_column_topo_chem'] = np.nan
                        l3.chem_fit.params['chem'] = np.nan
                        l3.chem_fit.mse_resid = np.nan
                        l3.chem_fit.rsquared = np.nan
        ...
        # zitong edit
        self.df['chem_lifetime'] = [-1/(l3.chem_fit.params['chem'])/3600 if hasattr(l3, 'chem_fit') else np.nan for l3 in self]
        self.df['chem_rmse'] = [np.sqrt(l3.chem_fit.mse_resid) if hasattr(l3, 'chem_fit') else np.nan for l3 in self]
        self.df['chem_r2'] = [l3.chem_fit.rsquared if hasattr(l3, 'chem_fit') else np.nan for l3 in self]
