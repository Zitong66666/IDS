...
class Level3_Data(dict):
    ...
    def fit_topography(self,mask=None,min_windtopo=None,max_windtopo=None,
        ...
        if self.instrum == 'TROPOMI' and self.product == 'NO2':
            ...
            outlier_std = outlier_std or 2
            fit_chem = fit_chem or False
        elif self.instrum == 'IASI' and self.product == 'NH3':
            min_windtopo = min_windtopo or 0.001
            max_windtopo = max_windtopo or 0.1
            max_iter = max_iter or 2
            outlier_std = outlier_std or 2
        ...
        self['wind_column_topo'] = wc_topo * region_mask # zitong edit: * region_mask, wind_column+X*wind_topo
        if if_xyrs:
    ...
    def fit_chemistry(self,mask=None,min_windtopo=None,max_windtopo=None,
        ...
        chem_mask = (wt >= min_windtopo) & (wt <= max_windtopo) & (wc <= max_wind_column) & (ww >= min_column_amount)
        if mask is not None:
        ...
        self['wind_column_topo_chem'] = wc_chem * region_mask # zitong edit
    ...
    def plot(self,plot_field=None,scale=None,
        ...
        if self.proj is None and 'lonmesh' not in self.keys():
            if scale == 'log':
                ...
                pc = ax.pcolormesh(*F_center2edge(xgrid,ygrid),plotdata,transform=ccrs.PlateCarree(),
                           alpha=kwargs['alpha'],cmap=kwargs['cmap'],vmin=kwargs['vmin'],vmax=kwargs['vmax'])#vmin=kwargs['vmin'],vmax=kwargs['vmax']
        else:
        ...
        if draw_colorbar:
            cb = plt.colorbar(pc,ax=ax,label=plot_field,shrink=kwargs['shrink'])
            # plt.clim(-0.0005, 0.0005) #zitong add
        else:
            cb = None
    ...
    def fit_chemistry(self,resample_rule=None,half_running_window=0,return_resampled=False,**kwargs):
        if resample_rule is None:
            ...
            for l3,(ind,sub_df) in zip(l3s_resampled,resampler.__iter__()):
                # print(l3.start_python_datetime)
                l3.fit_chemistry(**kwargs)
                for irow,row in sub_df.iterrows():
        ...
        print('here')
        print(self[0]['wind_column_topo_chem'])
        self[0].plot(plot_field='wind_column_topo_chem',saving_path='temp1.png')
